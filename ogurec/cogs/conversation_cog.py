...
class ConversationCog(commands.Cog):
    ...
    async def generate_report(self):
        ...
        - Время уже посчитано. Пиши его ровно как в данных. Никаких секунд, сложений, формул и дробных часов.
        - уложись в 2000 символов. ЭТО ОБЯЗАТЕЛЬНО.
        - Говори про всех из этого списка, никого не пропусти, в том же порядке.

        Формат (Discord Markdown), строго такой:
        ## Отчёт за вчера <эмодзи>
        **<@id>** — всего <время>
        - **<игра>** — <время>. <короткая подколка, одно предложение>
        (пустая строка между пользователями)
        > Итог: всего на сервере <время>. <одна финальная подколка>
        """,
                }
            )
